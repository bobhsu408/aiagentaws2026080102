...
| 拿不到簽章 URL | 無法取得語音服務授權（HTTP 400 / 500） |
| WebSocket 連線失敗 | 語音服務連線失敗（WebSocket error） |
| Transcribe 回例外 | 語音服務回報錯誤（例外型別） |

### 失敗重試與現場診斷

不採用「一次失敗就永久停用」的做法，改用連續失敗計數 `VOICE_MAX_FAILURES = 2`：

- 失敗 1 次 → 顯示原因，下次點擊仍會真的嘗試連線。
- 連續失敗 2 次 → 按鈕標成示範模式（`.demo-only` 虛線框）。
- 再點一次 → 跳過連線直接播示範動畫，但**同時把計數歸零**，所以下一次點擊又會重試。
- 成功收到 `TranscriptEvent` → 計數歸零並解除虛線框。

理由：若一次失敗就鎖定整個頁面 session，後端部署完成或修好之後，使用者不重新整理頁面
就永遠連不上，容易誤判成「功能壞了」。

訊息尾端一律附錯誤碼（同時 `console.warn` 一份），對照表：

| 畫面訊息尾端 | 病因 |
|------|------|
| `HTTP 400` | Lambda 還是舊版，不認得 `action=voice_url` → 需 `cdk deploy` |
| `HTTP 500` | Lambda 已更新但角色缺 `transcribe:StartStreamTranscriptionWebSocket` |
| `WebSocket error` | 簽章取得成功但連不上 Transcribe（網路或簽章過期） |
| 例外型別名稱 | Transcribe 服務端回報的錯誤（參數不符等） |
...
| 前端邏輯 | jsdom，46 項斷言 | 全數通過 |
| 失敗重試行為 | jsdom 模擬連續失敗與復原 | 通過。第 1 次不鎖、第 2 次標示範模式、第 3 次播動畫且提示可重試 |
